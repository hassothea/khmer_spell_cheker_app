...
class SpellChecker:
    ...
    def __init__(self, model_path: str = None) -> None:
        ...
        self.root = ET.parse(model_path).getroot().find("node") #.getchildren()[0]

        if self.root is None:
            raise ValueError("Invalid model.xml — no <node> found under <root>")
    ...
    def suggest(self, word: str, num_suggestions: int = 3, N: int = 2) -> List[str]:
        ...
        while len(stack) != 0:
            curr_node = stack.pop()
            curr_node_word = curr_node.attrib.get("word")
            if not curr_node_word:
                continue  # skip nodes without a word
            distance_curr_node = edit_distance(word, curr_node_word)
            ...
            children = curr_node.find("children")
            if children is not None:
                for child in children: 
                    child_weight = int(child.attrib.get("weight", 0))
